...
| **3. Flow State (Clustering)** | **Context:** Same Location (No Travel). Evaluates efficiency when tasks are co-located (e.g., Gym → Sauna).<br><br>• **Fragmented vs. Blocked:** Evaluates the gap to adjacent tasks.<br><br>• **Reward:** If the gap is <15 mins, we assume "Batching" (efficient).<br><br>• **Penalty:** If the task creates an "island" in the middle of free time, we penalize it to preserve deep work blocks. | **+15 pts** (Batching)<br><br>**-5 pts** (Island) |
| **4. Resilience Buffers** | **Context:** Different Location (Travel Required). Analyzes the safety buffer after accounting for travel time.<br><br>• **The "Goldilocks" Zone:** Analyzes the gap *before* the task starts.<br><br>• **Dangerous (<15m):** High penalty. Risk of cascading delays.<br><br>• **Ideal (15-45m):** Max reward. Perfect for travel/prep.<br><br>• **Neutral (>45m):** No impact. | **+10 pts** (Ideal)<br><br>**-10 pts** (Risky) |
